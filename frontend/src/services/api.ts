...
import { enrichDoctorsWithRotation, generatePerpetualSlots } from './rotationEngine';
...
class ApiClient {
  ...
  async getAvailableSlots(doctorId: string, date: string, patientId?: string): Promise<AvailableSlot[]> {
    ...
    const doctor = DEFAULT_PHYSICIANS.find(d => d.id === doctorId) || DEFAULT_PHYSICIANS[0];
    return generatePerpetualSlots(doctor, date, { currentPatientId: patientId });
  }
